...
ContentManager.prototype.getContentPlugin = function(type, cb) {
  ...
  pluginmanager.getManager().getPlugin('content', type, function(error, pluginInfo) {
    if (error) {
  ...
  */
  ...
  rest.post('/content/clipboard/copy', function(req, res, next) {
    ...
    database.getDatabase(function(error, db) {
      ...
      // Remove any existing clipboard data for this user
      async.series([
        function(callback) {
        ...
        function(callback) {
          db.retrieve(hierarchy[referenceType], { _id: id }, function(error, results) {
            if (error) {
        ...
        // TODO do we want to return 200 status whatever?
        if (err) {
          logger.log('error', err);
          return res.status(200).json({ success: false, message: 'Error adding clipboard data' });
        }
  ...
  rest.post('/content/clipboard/paste', function(req, res, next) {
    ...
    // not quite the inverse of above
    var collectionToType = {
      'contentObjects': 'contentobject',
      'articles': 'article',
      'blocks': 'block',
      'components': 'component'
    ...
    database.getDatabase(function(error, db) {
      ...
      db.retrieve('clipboard', {id: id, createdBy: user._id}, function(error, results) {
        ...
        if (results.length == 1) {
          ...
          async.series([
              ...
              function(callback) {
                db.retrieve(parentRelationship[clipboard.referenceType], {_id: parentId}, function(error, results) {
                  ...
                  if (clipboard.referenceType == 'contentObjects' && (clipboard.contentObjects[0]._courseId.toString() === clipboard._courseId.toString())) {
              ...
              // Set the sort order
              function(callback) {
                if (sortOrder && clipboard[clipboard.referenceType].length == 1) {
              ...
              function(callback) {
                async.eachSeries(Object.keys(parentRelationship), function(level, cb) {
                  map[level] = {};
              ...
              // Do paste
              function(callback) {
                async.eachSeries(Object.keys(parentRelationship), function(contentType, cb) {
                  if(clipboard[contentType].length < 1) {
                    return cb();
                  }
                  async.eachSeries(clipboard[contentType], function(item, cb2) {
                    var parentCollection = typeToCollection[parentRelationship[contentType]];
                    var previousId = item._id.toString();
                    var previousParentId = item._parentId.toString();
                    var mappedId = map[parentCollection][previousParentId];
                    var newParentId = (typeof mappedId !== 'undefined' && mappedId != null) ? mappedId : parentObject._id;

                    delete item._id;
                    item._parentId = newParentId;

                    self.create(collectionToType[contentType], item, function(error, newItem) {
                      if (error) return cb2(error);

                      map[contentType][previousId] = newItem._id;
          ...
          return res.status(200).json({ success: false, message: 'More than one clipboard found' });
        }
      }, tenantId);
    });
  });
...
ContentManager.prototype.onDatabaseCreated = function(db, next) {
  var self = this;
  pluginmanager.getManager().getPlugin('content', function(error, plugins) {
    async.eachSeries(Object.keys(plugins), function(key, cb) {
...
function copyCourseAssets(db, data, callback) {
  ...
  */
  ...
  db.retrieve('courseasset', {_contentTypeId: data.previousId}, function(err, courseAssets) {
    ...
    async.each(courseAssets, function(courseAsset, cb) {
      ...
      db.create('courseasset', courseAsset, cb);
    }, callback);
  });
}
