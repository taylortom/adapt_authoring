...
function checkPrerequisites(onPrerequisitesPassed) {
    // HACK
    var loggerLevel = logger.logger.transports.console.level;
    logger.logger.transports.console.level = 'error';
    ...
    async.parallel([
        function checkNodeVersion(callback) {
          var installedNodeVersion = process.versions.node;
          var requiredNodeVersion = packageJson.engines.node;

          if(!semver.satisfies(installedNodeVersion, requiredNodeVersion)) {
            var error = new Error('Your installed node version (' + installedNodeVersion + ') is incompatible with the Adapt authoring tool, ' + requiredNodeVersion + ' required.');
          }
        ...
        function checkDB(callback) {
            // MongoDB is installed and can be connected to
            callback();
        },
            ...
            async.parallel([
                function git(cb) {
                    exec('git --version', function(error) {
                        if(error) cb(new Error('Git not found. Please install it.'));
                        async.parallel([
                            function gitProto(cb2) {
                                exec('git ls-remote git://github.com/adaptlearning/adapt_framework.git', function(error) {
                                    if(error) cb2("Couldn't connect to GitHub.com using the GIT protocol. Please check your network configuration.");
                                    cb2();
                                });
                            },
                            function httpsProto(cb2) {
                                exec('git ls-remote git://github.com/adaptlearning/adapt_framework.git', function(error) {
                                    if(error) cb2("Couldn't connect to GitHub.com using the HTTPS protocol. Please check your network configuration.");
                                    cb2();
                                });
                            }
                        ], callback);
                ...
                function gruntCli(cb) {
                    exec('grunt --version', function(error) {
                        if(error) cb(new Error('Grunt not found. Please install it.'));
                        cb();
                    });
                },
                function adaptCli(cb) {
                    exec('adapt --version', function(error) {
                        if(error) cb(new Error('Adapt CLI not found. Please install it.'));
                        cb();
                    });
                },
                function mongo(cb) {
        ...
        // HACK
        logger.logger.transports.console.level = loggerLevel;
        logger.log('info', 'Prerequisites OK!');

        process.exit(1);
        onPrerequisitesPassed();
    });
}
...
Origin.prototype.run = function (options) {
  ...
  this.once('modulesReady', function (app) {
      checkPrerequisites(function onPrerequisitesPassed() {
          try {
