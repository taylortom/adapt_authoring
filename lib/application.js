...
Origin.prototype.modulePreloadEventHandler = function(name, status, response) {
  ...
  this._preloadModules.some(function(mod) {
    if (name === mod.moduleName) {
      if ('undefined' === typeof response) response = false;
...
Origin.prototype.createServer = function(options, cb) {
  ...
  require('./database').getDatabase(function(err, db) {
    ...
    server.use(session({
      ...
      store: new MongoStore({ mongooseConnection: db.conn }),
      resave: true,
      cookie: {
        signed: true,
      ...
      Object.keys(plugins[directory]).forEach(function(name) {
        try {
          ...
          if ('function' == typeof plugin.getMiddleware) {
            plugin.getMiddleware(server, callback);
          }
        } catch (err) {
...
Origin.prototype.startServer = function(options) {
  ...
  async.series([
    function(callback) {
      ...
      if (versionFile) {
        installedServerVersion = versionFile.adapt_authoring;
    ...
    function(callback) {
      ...
      request({
        ...
        if (error) {
          ...
          if (tagInfo) latestServerTag = tagInfo[0].name;
         }
         callback();
      });
    },
    function(callback) {
      ...
      request({
        ...
        if (error) {
          ...
          var tagInfo = JSON.parse(body);
          if (tagInfo) latestFrameworkTag = tagInfo[0].name;
        }
        callback();
      });
    },
    function(callback) {
      ...
      if (installedServerVersion == latestServerTag) {
        logger.log('info', chalk.green('%s %s'), app.polyglot.t('app.productname'), installedServerVersion);
      } else {
        logger.log('info', chalk.yellow('You are currently running %s %s - %s is now available'), app.polyglot.t('app.productname'), installedServerVersion, latestServerTag);
